      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex items-center space-x-4 hover:shadow-md transition-shadow">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-gray-900">${summary?.totalInventoryValue?.toLocaleString('es-AR') || 0}</h3>
          </div>
        </div>
      </div>
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stockByCategory || []}>
                ...
                <XAxis dataKey="categoryName" axisLine={false} tickLine={false} tick={{ fill: '#6b7280' }} />
              ...
              <div key={mov.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                <div>
                  ...
                  <p className="text-xs text-gray-500 capitalize">
                    {formatDistanceToNow(new Date(mov.createdAt), { addSuffix: true, locale: es })} • {mov.type === 1 ? 'Ingreso' : mov.type === 2 ? 'Salida' : 'Ajuste'}
                  </p>
                </div>
