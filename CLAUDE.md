...
- ⚠️ **Acción del usuario antes de mergear a `main`**: crear el secret `CORS_ORIGINS` en GitHub (Settings → Secrets → Actions) con la URL real del frontend. `deploy.yml` ahora lo sincroniza al `.env` del server en cada deploy — si el secret no existe, el step aborta antes de tocar el contenedor (no rompe producción en silencio), pero el deploy no va a completar hasta que se cree.
- La migración `AddCategoryNameUniqueIndexAndUserRefreshTokenIndex` agrega un índice único a `Categories.Name`. Si la base de producción tuviera dos categorías con el mismo nombre, fallaría al aplicarse en el startup y el contenedor no levantaría. Como la DB de producción es solo datos de demo (nada real todavía), no hace falta reconciliar nada a mano si llegara a chocar — ver `deploy/ROLLBACK.md` para el procedimiento completo (incluye la advertencia de que `DatabaseSeeder` no corre en producción).
...
- CSP del frontend con `'unsafe-inline'` + `'unsafe-eval'` en `script-src` (`next.config.mjs`) — aceptado por ahora. Arreglarlo bien requiere CSP con nonces, que a su vez requiere `middleware.ts` server-side — mismo trade-off ya documentado para el auth guard client-side.
