...
- CSP del frontend con `'unsafe-inline'` + `'unsafe-eval'` en `script-src` (`next.config.mjs`) — aceptado por ahora (ver `### Seguridad y performance` abajo). Arreglarlo bien requiere CSP con nonces, que a su vez requiere `middleware.ts` server-side — mismo trade-off ya documentado para el auth guard client-side. Ahora que Next está en 16, es el próximo paso lógico (mejor soporte nativo de nonces que en 14).
